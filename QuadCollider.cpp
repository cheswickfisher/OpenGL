#include "QuadCollider.h"
namespace WIP_Polygon {
	QuadCollider::QuadCollider() :
		Collider{ glm::vec3(1.0f), glm::vec3(0.0f), glm::vec3(0.0f, 0.0f, 0.0f), &quad },
		quad{ WIP_Polygon::Quad() }
	{
	}
	QuadCollider::QuadCollider(glm::vec3 _scale) :
		Collider{ _scale, glm::vec3(0.0f), glm::vec3(0.0f, 0.0f, 0.0f), &quad },
		quad{ WIP_Polygon::Quad() }
	{
	}
	QuadCollider::QuadCollider(glm::vec3 _scale, glm::vec3 _position) :
		Collider{ _scale, _position, glm::vec3(0.0f, 0.0f, 0.0f), &quad },
		quad{ WIP_Polygon::Quad() }
	{
	}
	QuadCollider::QuadCollider(glm::vec3 _scale, glm::vec3 _position, glm::vec3 _rotation) :
		Collider{ _scale, _position, _rotation, &quad },
		quad{ WIP_Polygon::Quad() }
	{
	}

	void QuadCollider::DrawAxes()  {

	}
	glm::mat3 QuadCollider::CalculateInertiaTensor()  {
		//formulas from https://en.wikipedia.org/wiki/Moment_of_inertia#Inertia_tensor
		float ex2 = 4.0f * e.x * e.x;
		float ey2 = 4.0f * e.y * e.y;
		//float ez2 = 4.0f * e.z * e.z;
		float mass = aabb->rigidbody->mass;
		float Ixx = 1.0f / 12.0f * mass * (ey2);
		float Iyy = 1.0f / 12.0f * mass * (ex2);
		float Izz = 1.0f / 12.0f * mass * (ex2 * ey2);
		glm::mat3 i_t = glm::mat3(1.0f);
		i_t[0][0] = Ixx;
		i_t[1][1] = Iyy;
		i_t[2][2] = Izz;
		//std::cout << "1. i_t[0,0]->" << i_t[0][0] << " i_t[1,1]->" << i_t[1][1] << " i_t[2,2]->" << i_t[2][2] << "\n";
		glm::vec3 position = aabb->rigidbody->position;
		glm::quat rot = aabb->rigidbody->rotation;
		//transform inertia tensor to world space
		glm::mat3 R = glm::toMat3(rot);
		i_t = R * i_t * glm::transpose(R); //rotation
		//std::cout << "2. i_t[0,0]->" << i_t[0][0] << " i_t[1,1]->" << i_t[1][1] << " i_t[2,2]->" << i_t[2][2] << "\n";
		i_t += mass * (glm::dot(position, position) * glm::mat3(1.0f) - glm::outerProduct(position, position)); //translation
		//std::cout << "3. i_t[0,0]->" << i_t[0][0] << " i_t[1,1]->" << i_t[1][1] << " i_t[2,2]->" << i_t[2][2] << "\n";
		return i_t;
	}

}